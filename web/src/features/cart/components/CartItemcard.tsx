...
export const CartItemCard = ({ item, index }: CartItemCardProps) => {
  ...
  // DNA signature: prevents stale state when items reorder after removal
  ...
  return (
    <div className={`relative flex flex-col p-3.5 bg-zinc-900/85 border border-white/10 rounded-2xl shadow-[0_8px_24px_rgba(0,0,0,0.25)] transition-all duration-300 ${isRemoving ? 'opacity-0 scale-95 translate-x-4' : 'opacity-100 scale-100 translate-x-0'}`}>
      <CartItemHeader
        title={product.title}
        price={product.price}
        imageSrc={imageSrc}
      ...
      {availableAddons.length > 0 && (
        <button onClick={() => setShowExtras(!showExtras)} className="mt-3 flex w-full items-center justify-center gap-1.5 rounded-xl border border-white/5 bg-white/5 py-2 text-xs font-semibold text-white/60 hover:bg-white/10 hover:text-white/90 transition-all">
      ...
      {showExtras && (
        <CartItemExtrasPanel
          ...
          onToggleShowAll={() => setShowAllAddons(p => !p)}
