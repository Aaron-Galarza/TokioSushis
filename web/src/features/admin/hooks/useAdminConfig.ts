...
import {
  ...
  addSpecialZone as apiAddZone, deleteSpecialZone as apiDeleteZone,
...
const DEFAULT_SCHEDULE: DaySch[] = DAYS.map(day => ({
  day,
  openTime: '12:00',
  closeTime: '23:00',
  isStoreOpen: true,
...
export function useAdminConfig() {
  ...
  const reload = useCallback(async () => {
    try {
      ...
      // 🛠️ SOLUCIÓN MAESTRA: Evaluamos las dos claves posibles que puede estar mandando el Backend
      ...
      if (dbSchedule.length > 0) {
        setSchedule(DAYS.map((day, i) => {
          ...
          return {
            day,
            openTime: match?.openTime ?? '12:00',
            closeTime: match?.closeTime ?? '23:00',
  ...
  const toggleEmergency = async () => {
    try {
      await apiToggleEmergency();
      await reload();
    } catch (err) {
      console.error('toggleEmergency:', err);
    }
  };
...
const toggleRain = async () => {
    try {
      ...
      // Aseguramos que extraRain sea un número válido mayor o igual a 0
      const cleanExtraRain = isNaN(extraRain) || extraRain < 0 ? 0 : Number(extraRain);
      
      await patchRain(next, cleanExtraRain);
      setIsRaining(next);
    } catch (err) {
      console.error('Error al cambiar el estado de lluvia:', err);
      // Opcional: Podés llamar a reload() acá para revertir la UI si falló la API
    }
  };
  ...
  const addZone = async () => {
    const lat = parseFloat(nZone.lat), lng = parseFloat(nZone.lng);
    const r = parseFloat(nZone.radiusMeters), price = parseFloat(nZone.price);
    if (!nZone.name || isNaN(lat) || isNaN(lng) || !r || r <= 0 || isNaN(price) || price < 0) {
      setZErr('Completá todos los campos'); return;
    }
    setZones(await apiAddZone({ name: nZone.name, lat, lng, radiusMeters: r, price }));
    setNZone({ name: '', lat: '', lng: '', radiusMeters: '', price: '' }); setZErr(''); setShowZoneForm(false);
  };

  const deleteZone = async (id: string) => { setZones(await apiDeleteZone(id)); };
