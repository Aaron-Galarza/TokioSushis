...
interface Props {
  menu: {
    ...
    addons: any[];
    aForm: { title: string; price: string; categories: string[] };
    ...
    products: any[];
    pForm: {
      title: string; price: string; category: string; description: string;
      image: string; controlStock: boolean; stock: string;
    };
...
// Helper: toggle un id dentro del array de categorías seleccionadas
function toggleCatId(current: string[], id: string): string[] {
      ...
      <div className="grid md:grid-cols-2 gap-5">
        ...
        <AdminCard>
          ...
          <div className="flex gap-2 mb-3">
            <AdminInput
              ...
              value={menu.cForm.name}
              onChange={e => menu.setCForm(p => ({ ...p, name: e.target.value }))}
              className="py-2"
            />
            <AdminInput
              type="number"
              placeholder="Orden"
              value={menu.cForm.order}
              onChange={e => menu.setCForm(p => ({ ...p, order: e.target.value }))}
              className="w-20 py-2"
            />
            <button
              onClick={menu.saveCat}
              className="bg-primary text-black font-bold px-3 py-2 rounded-lg text-xs hover:bg-primary/90 active:scale-95 transition-all whitespace-nowrap"
            >
        ...
        <AdminCard>
          ...
          <div className="flex gap-2 mb-2">
            <AdminInput
              placeholder="Nombre"
              value={menu.aForm.title}
              onChange={e => menu.setAForm(p => ({ ...p, title: e.target.value }))}
              className="py-2"
            />
            <AdminInput
              type="number"
              placeholder="Precio"
              value={menu.aForm.price}
              onChange={e => menu.setAForm(p => ({ ...p, price: e.target.value }))}
              className="w-24 py-2"
            />
            <button
              onClick={menu.saveAddon}
              className="bg-primary text-black font-bold px-3 py-2 rounded-lg text-xs hover:bg-primary/90 active:scale-95 transition-all whitespace-nowrap"
            >
          ...
          {/* Selector de categorías (multi-toggle con chips) */}
          <div className="mb-3">
            <p className="text-[10px] text-white/30 uppercase tracking-wider mb-1.5 pl-0.5">
              Categorías que lo reciben
              <span className="normal-case ml-1 text-white/20">(vacío = todas)</span>
            </p>
            <div className="flex flex-wrap gap-1.5">
              {menu.cats.filter(c => c.active).map(c => {
                const selected = menu.aForm.categories.includes(c._id);
                return (
                  <button
                    key={c._id}
                    type="button"
                    onClick={() =>
                      menu.setAForm(p => ({
                        ...p,
                        categories: toggleCatId(p.categories, c._id),
                      }))
                    }
          ...
          <div className="flex flex-col gap-1.5 max-h-52 overflow-y-auto overscroll-contain scrollbar-none">
            {menu.addons.map(a => {
              // Resolvemos los nombres de las categorías asignadas para mostrarlas
              const catLabels: string[] = (a.categories ?? []).map((c: any) => {
                if (typeof c === 'object') return c.name;
                return menu.cats.find((mc: any) => mc._id === c)?.name ?? c;
              });

              return (
                <div
                  key={a._id}
                  className="flex items-center justify-between bg-[#1A1A1A] border border-white/10 rounded-lg px-3 py-2"
                >
                  <div className="flex flex-col min-w-0">
                    <span className={`text-sm ${a.active ? 'text-white' : 'text-white/30 line-through'}`}>
                      {a.title || a.name}
                    </span>
                    {catLabels.length > 0 ? (
                      ...
                      <span className="text-[10px] text-white/20 italic">Todas las categorías</span>
                    )}
                  </div>
      ...
      <div className="grid md:grid-cols-[2fr_3fr] gap-5">
        <AdminCard>
          <h2 className="font-semibold text-white text-sm mb-3">
            {menu.pEditId ? 'Editar Producto' : 'Nuevo Producto'}
          </h2>
          {menu.pErr && <p className="text-red-400 text-xs mb-2">{menu.pErr}</p>}
          <div className="flex flex-col gap-2">
            <AdminInput
              placeholder="Nombre del producto"
              value={menu.pForm.title}
              onChange={e => menu.setPForm(p => ({ ...p, title: e.target.value }))}
              className="py-2"
            />
            <div className="flex gap-2">
              <AdminInput
                type="number"
                placeholder="Precio"
                value={menu.pForm.price}
                onChange={e => menu.setPForm(p => ({ ...p, price: e.target.value }))}
                className="flex-1 py-2"
              />
              <AdminSelect
                value={menu.pForm.category}
                onChange={e => menu.setPForm(p => ({ ...p, category: e.target.value }))}
                className="flex-1 py-2"
              >
            ...
            <textarea
              ...
              value={menu.pForm.description}
              onChange={e => menu.setPForm(p => ({ ...p, description: e.target.value }))}
              rows={2}
            ...
            <AdminInput
              ...
              value={menu.pForm.image}
              onChange={e => menu.setPForm(p => ({ ...p, image: e.target.value }))}
              className="py-2"
            />
            <div className="flex items-center gap-3 flex-wrap">
              <label className="flex items-center gap-1.5 text-xs text-white/50 cursor-pointer">
                <input
                  type="checkbox"
                  checked={menu.pForm.controlStock}
                  onChange={e => menu.setPForm(p => ({ ...p, controlStock: e.target.checked }))}
                  className="accent-primary"
                /> Stock
              </label>
              {menu.pForm.controlStock && (
                <AdminInput
                  type="number"
                  placeholder="Cant."
                  value={menu.pForm.stock}
                  onChange={e => menu.setPForm(p => ({ ...p, stock: e.target.value }))}
                  min={0}
                  className="w-20 py-2"
                />
              )}
            ...
            <div className="flex gap-2">
              <button
                onClick={menu.saveProduct}
                className="flex-1 bg-primary text-black font-bold py-2 rounded-lg text-sm hover:bg-primary/90 active:scale-95 transition-all"
              >
                {menu.pEditId ? 'Guardar cambios' : 'Crear producto'}
              </button>
              {menu.pEditId && (
                <button
                  onClick={menu.cancelProduct}
                  className="bg-white/5 text-white/50 px-3 py-2 rounded-lg text-sm hover:text-white"
                >✕</button>
              )}
            </div>
          </div>
          ...
          <div className="flex flex-col gap-1.5 max-h-72 overflow-y-auto overscroll-contain scrollbar-none">
            {menu.products.map((p: any) => {
              const catName = typeof p.category === 'object'
                ? p.category?.name
                : (menu.cats.find((c: any) => c._id === p.category)?.name ?? '—');
                ...
                <div key={p._id} className="flex items-center gap-3 bg-[#1A1A1A] border border-white/10 rounded-lg px-3 py-2.5">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <p className={`text-sm font-semibold truncate ${p.active ? 'text-white' : 'text-white/30 line-through'}`}>
                        {p.title}
                      </p>
                      {p.controlStock && (
                        <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full shrink-0
                          ${(p.stock ?? 0) > 0 ? 'bg-green-900/40 text-green-300' : 'bg-red-900/40 text-red-300'}`}>
                          {p.stock ?? 0}
                        </span>
                      )}
                    </div>
                    <p className="text-[11px] text-white/30">
                      {catName} · <span className="text-primary">${p.price?.toLocaleString('es-AR')}</span>
                    </p>
                  </div>
